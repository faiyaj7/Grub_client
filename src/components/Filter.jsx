...
const Filter = ({ filterData, setFilterData }) => {
  ...
  return (
    <>
      <button
        className="block lg:hidden bg-pink-600 px-5 py-2 text-white rounded-md ml-auto mr-4 mt-5"
      ...
      <aside
        ...
        } lg:block border-black/10 pt-10 px-5 w-full lg:w-[25%] h-[600px] overflow-y-scroll`}
      >
          ...
          {Offer.map(({ name, id, value }) => (
            <div className="flex items-center justify-start gap-2" key={name}>
              <div className="relative grid place-items-center">
                ...
                />
                <svg
                  className="absolute w-3 h-3 mt-1 hidden peer-checked:block pointer-events-none text-white"
              ...
              <div className="relative grid place-items-center">
                <input
                  ...
                  onChange={() =>
                    setFilterData((prev) => ({
                      ...prev,
                      selectedCuisines: prev.selectedCuisines.includes(name)
                        ? prev.selectedCuisines.filter(
                            (cuisine) => cuisine !== name
                          )
                        : [...prev.selectedCuisines, name],
                    }))
                  }
                  className="relative peer shrink-0
                    appearance-none w-5 h-5 border-2 border-pink-500 rounded-sm bg-white mt-1 checked:bg-pink-600 checked:border-0"
